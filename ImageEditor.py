...
class Example(Frame):
    ...
    def initMenubar(self):
        ...
        #filtersMenu.add_command(label="Негатив", command=self.convertToNegative)
        ...
        #filtersMenu.add_command(label="Соляризация", command=self.solarize)
        ...
        #filtersMenu.add_command(label="Градиент радуги", command=self.rainbowGradient)
    ...
    def initFrameForButtons(self):
        ...
        #labelframeFormat = LabelFrame(toolKitFrame, text="Формат изображения")
        #labelframeFormat.pack(fill="both", expand="yes")
        ...
        """
        convertToNegativeButton = Button(toolKitFrameFilters, text="Негатив", \
                              background=backgroundColour, \
                              width=buttonWidth, height=buttonHeight, \
                              command=self.convertToNegative)
        convertToNegativeButton.grid(row=2, column=0)
        """
    ...
    def convertToNegative(self):
        ...
        if self.parent.canvas.data.image != None:
            ...
            # Recombine back to RGB image
            negativeImage = Image.merge('RGB', (r, g, b))

            self.parent.canvas.data.image = negativeImage
            self.parent.canvas.data.undoQueue.append(self.parent.canvas.data.image.copy())
            self.parent.canvas.data.imageForTk = self.makeImageForTk()
            self.drawImage()
    """
    ...
    def convertGray(self):
        ...
        if self.parent.canvas.data.image != None:
            ...
            self.drawImage()
    """
    def rainbowGradient(self):
        self.parent.canvas.data.colourPopToHappen = False
        self.parent.canvas.data.cropPopToHappen = False
        self.parent.canvas.data.drawOn = False
        if self.parent.canvas.data.image != None:
            image = self.parent.canvas.data.image
            image_bw = cv2.IMREAD_GRAYSCALE(self.parent.canvas.data.imageLocation)
            image_rgb = cv2.imread(self.parent.canvas.data.imageLocation)
            ...
            rainbowGradientImage = image_rgb

            self.parent.canvas.data.image = rainbowGradientImage
            self.parent.canvas.data.undoQueue.append(self.parent.canvas.data.image.copy())
            self.parent.canvas.data.imageForTk = self.makeImageForTk()
            self.drawImage()
            # cv.WaitKey(0)
